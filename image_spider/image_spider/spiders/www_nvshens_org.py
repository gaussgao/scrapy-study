...
class WwwNvshensOrgSpider(scrapy.Spider):
    name = 'nvshens'
    allowed_domains = ['www.nvshens.org']
    # 初始URL
   # start_urls = ['https://www.zbjuran.com/mei/']
   # scrapy crawl xh --nolog
    start_urls = ['https://www.nvshens.org/girl/20625/']
    # 设置一个空集合
    #url_set = set()
    ...
    def parse(self, response):
        ...
        if (False == WwwNvshensOrgSpider.url_list_db.query(response.url,1)):

            WwwNvshensOrgSpider.url_list_db.update(response.url)
            ...
            if True :
                ...
                allPics = response.xpath('//ul[@id="hgallery"]/img')

                #print ("2.get img ing ...... ",allPics)
                for pic in allPics:
                    # 分别处理每个图片，取出名称及地址
                    #print ("2.get img ing ...... ",sys._getframe().f_lineno )
                    item = PicItem()
            ...
            for url in urls:
                ...
                if url.startswith("/g/") or url.startswith("/girl/") :
                    #and url.endswith(".html"):
                    
                    url_whole = "https://www.nvshens.org/"+url

                    if WwwNvshensOrgSpider.url_list_db.query(url_whole):
                        #print ("Exist:",url,url_whole)
                        pass
                        
                    else:
                        #XhSpider.url_set.add(url_whole)
                        WwwNvshensOrgSpider.url_list_db.insert(url_whole)
        ...
        url_whole =  WwwNvshensOrgSpider.url_list_db.query_data()
