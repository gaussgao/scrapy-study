...
class DuotooSpider(scrapy.Spider):
    name = 'duotoo'
    allowed_domains = ['www.duotoo.com']
    # 初始URL
   # start_urls = ['https://www.zbjuran.com/mei/']
   # scrapy crawl xh --nolog
    start_urls = ['https://www.duotoo.com/neiyimeinv/29395.html']
    # 设置一个空集合
    #url_set = set()

    url_list_db = DemoDB("url_list1.slqite3.db",False)

    def parse(self, response):
        
        if (False == DuotooSpider.url_list_db.query(response.url,1)):

            DuotooSpider.url_list_db.update(response.url)
            ...
            if True :
                ...
                allPics = response.xpath('//article/section/div/p/a/img')
            ...
            for url in urls:
                ...
                if  url.startswith("http") and url.endswith(".html"):
                    ...
                    if DuotooSpider.url_list_db.query(url_whole):
                        #print ("Exist:",url,url_whole)
                        pass
                    else:
                        #XhSpider.url_set.add(url_whole)
                        DuotooSpider.url_list_db.insert(url_whole)
                    ...
                    url_whole = "https://www.duotoo.com/"+url
        ...
        url_whole =  DuotooSpider.url_list_db.query_data()
